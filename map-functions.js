const map = L.map("map", { scrollWheelZoom: true });
  ...
  // Aggiunto: setSelected(values, {silent}) per preselezionare da codice
  function createCheckboxDropdown(rootEl, opts){
    ...
    return {
      ...
      setSelected(arr, { silent = false } = {}){
        ...
        // rimuovi eventuali selezioni non presenti in values
        for (const v of [...selected]) if (!values.includes(v)) selected.delete(v);
        updateHeader();
        renderList();
  ...
  // ✅ Dropdown instances: region/province/city (multi + cascade)
  const ddRegion = createCheckboxDropdown(els.ddRegion, {
    placeholder: "Tutte le regioni",
    onChange: (arr) => {
      state.selectedRegions = new Set(arr);
      cascadeFromRegions(); // aggiorna province e città disponibili
      applyFilters();
    }
  });
  ...
  const ddProvince = createCheckboxDropdown(els.ddProvince, {
    ...
    onChange: (arr) => {
      ...
      // auto-seleziona (aggiungi) le regioni delle province selezionate
      const derivedRegions = new Set();
      for (const code of state.selectedProvinces) {
        const reg = provRegion(code);
        if (reg) derivedRegions.add(reg);
      }
      // unisci a regioni già selezionate manualmente
      const merged = new Set([...state.selectedRegions, ...derivedRegions]);

      // aggiorna regioni UI + stato (senza triggerare doppia cascata)
      ddRegion.addSelected([...derivedRegions], { silent: true });
      state.selectedRegions = merged;

      cascadeFromRegions(); // restringe province (coerenti) + città
      applyFilters();
    }
  });
  ...
  const ddCity = createCheckboxDropdown(els.ddCity, {
    ...
    onChange: (arr) => { state.selectedCities = new Set(arr); applyFilters(); }
  });
  ...
  // compute available sets based on selected regions/provinces
  function computeAvailableGeo(){
    const regionsSel = [...state.selectedRegions];
    const provincesSel = [...state.selectedProvinces];

    const provincesAvail = new Set();
    const citiesAvail = new Set();
    const regionsAvail = new Set();
    ...
    for (const item of state.items){
      ...
      if (reg) regionsAvail.add(reg);

      // apply current selections as constraints
      if (regionsSel.length > 0 && !regionsSel.includes(reg)) continue;
      if (provincesSel.length > 0 && !provincesSel.includes(code)) continue;

      if (code) provincesAvail.add(code);
      if (city) citiesAvail.add(city);
    }

    return { regionsAvail, provincesAvail, citiesAvail };
  }

  // rebuild province + city dropdown values based on region/province selections
  function cascadeFromRegions(){
    const { provincesAvail, citiesAvail } = computeAvailableGeo();

    // Province dropdown: values = solo province compatibili
    // Nota: valori nel dropdown provincia sono codici "MI", "RM"... ma visualizziamo "MI — Milano"
    ddProvince.setValues([...provincesAvail].map(code => code));
    // Aggiorna label visuale nel dropdown provincia: trasformiamo le voci in label estesa usando provLabel
    // (Trucco semplice: setValues con stringhe "MI — Milano", ma poi perdiamo il codice)
    // Quindi: teniamo il VALUE come "MI" ma mostriamo label estesa nel rendering:
    // -> lo facciamo modificando l'HTML dopo render: gestiamo in setProvinceLabels()
    setProvinceLabels();

    // Se alcune province selezionate non sono più disponibili, puliscile
    for (const code of [...state.selectedProvinces]) {
      if (!provincesAvail.has(code)) state.selectedProvinces.delete(code);
    }
    ddProvince.setSelected([...state.selectedProvinces], { silent: true });

    // Città dropdown: solo città compatibili
    ddCity.setValues(citiesAvail);
    for (const c of [...state.selectedCities]) {
      if (!citiesAvail.has(c)) state.selectedCities.delete(c);
    }
  ...
  function rebuildFilters(features){
    ...
    // applica cascata in base a eventuali selezioni già presenti
    cascadeFromRegions();
  }
  ...
  function applyFilters(){
    ...
    for (const item of state.items){
      ...
      // ✅ region/province/city filters (AND across dimensions)
      if (regionsSel.length > 0 && !regionsSel.includes(reg)) continue;
      if (provincesSel.length > 0 && !provincesSel.includes(code)) continue;
